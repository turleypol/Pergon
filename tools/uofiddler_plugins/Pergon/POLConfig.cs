...
namespace POLConfig
{
	public class POLConfigFile
	{
        ...
        {
            if (line[0] == '#' || (line.Length >= 2 && line.Substring(0, 2) == @"//"))
                return true;
				...
				while ((line = sr.ReadLine()) != null)
				{
					...
					if ( cur_elem == null )
                    {
                        ...
                        else
                        {
                            ...
                            {
                                cur_elem = new POLConfigElem(line);
                            }
                        }
                    }
					else
	...
	class POLConfigElem
	{
        ...
        List<POLConfigLine> _write_order = new List<POLConfigLine>(); // Stores the order of data (POLConfigLine)
        ...
        public List<POLConfigLine> Structure
        {
            get { return _write_order; }
        }
		...
		public int GetConfigInt(string property_name)
		{
			if (_properties.ContainsKey(property_name))
                return Convert.ToInt32(((POLConfigLine)_properties[property_name])._value);
			else
				return 0;
		}
		...
		{
			if (_properties.ContainsKey(property_name))
                return Convert.ToDouble(((POLConfigLine)_properties[property_name])._value, System.Globalization.NumberFormatInfo.InvariantInfo);
			else
				return 0.0;
		}
		...
		{
			if (_properties.ContainsKey(property_name))
				return Convert.ToString(((POLConfigLine)_properties[property_name])._value);
			else
				return "";
		}
